...
import org.copygrinder.pure.copybean.exception.{JsonInputException, CopygrinderRuntimeException, TypeValidationException}
  ...
  protected def calcTypeFields(): Option[Seq[CopybeanFieldDef]] = getFieldAsList("fields").flatMap { fields =>
    val fieldDefs = fields.map {
      case m: Map[_, _] =>
        ...
        val displayName = getStringOpt("displayName")
